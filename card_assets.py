...
def build_card_assets(png_bytes: bytes) -> tuple[tuple[str, bytes], ...]:
    """Build the two Wallet PNGs and PDF used by the original Mac flasher."""
    with tempfile.TemporaryDirectory(prefix="aircard-assets-") as temporary:
        ...
        subprocess.run(
            ["/usr/bin/sips", "-s", "format", "pdf", str(png_path),
             "--out", str(pdf_path)],
            check=True,
    ...
    return (*tuple((name, png_bytes) for name in PNG_ASSET_NAMES),
            (PDF_ASSET_NAME, pdf_bytes))
